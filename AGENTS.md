...
- 애플리케이션 기반, 브랜드·상품 카탈로그, 이미지 업로드와 상품 사진·트레이 사진 작업함 구현이 완료되었으며, 다음 단계는 라벨링 편집기입니다.
- 현재 실행 가능한 범위는 브랜드 선택, 브랜드·상품 관리, 상품 사진과 트레이 사진 업로드·조회·검색·삭제까지이며, 박스 라벨링은 아직 구현되지 않았습니다.
- 승인된 기술 스택은 React·TypeScript·Vite·Tailwind CSS 4·shadcn/ui, FastAPI·Python 3.13, SQLite·SQLAlchemy, 로컬 파일 저장소입니다.
...
- Pretendard와 BIXOLON Orange `#EE7203`를 사용하고 MVP에는 다크 모드를 추가하지 않습니다.
- 대표 행동은 검정 배경, 현재 선택과 포커스는 BIXOLON Orange로 표현합니다.
- Card를 반복해 화면을 나누지 않고 여백과 구분선을 우선합니다.
- 라벨링 화면에서는 관리 탐색을 숨기고 캔버스, 저장 상태, 상품 지정에 집중합니다.
- 상태는 색상에만 의존하지 않고 아이콘, 선 모양, 사용자 문구를 함께 사용합니다.
